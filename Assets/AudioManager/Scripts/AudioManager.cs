...
namespace AudioManager
{
  ...
  public sealed class AudioManager : Singleton<AudioManager>
  {
    ...
    public List<SoundBank> SoundsBank { get => soundsBank; }
    ...
    [SerializeField]
    private List<SoundBank> soundsBank = new List<SoundBank>();
    ...
    public float PlayEffectExtended(AudioClip audioClip, float audioVolume = 1.0f, bool loop = false, Transform position = null, float minDist = 1.0f, float maxDist = 4.5f, bool bypassListenerEffects = false)
    {
      ...
      GameObject poolElement = audioEffectsPool.PullFromPool("AudioSource");
      duration = 0.0f;
    ...
    /// <summary>
    /// Para rreproducir efectos por eventos de editor
    /// </summary>
    public void PlayEffectString(string audioName)
    {
    ...
    private void LoadDictionaries(Scene scene, LoadSceneMode mode)
    {
      ...
      for (int i = 0; i < soundsBank.Count; i++)
      {
        ...
        {
          if (string.Equals(soundsBank[i].Levels[j], scene.name) == true)
          {
      ...
      if (soundBank != null)
      {
        ...
        if (string.Equals(rootName, scene.name) == true)
        {
    ...
    {
      SceneManager.sceneLoaded += LoadDictionaries;
    }
    ...
    private void Start()
    {
      if (audioEffectsPool != null)
        audioEffectsPool.InitiatePools();
      else
        Debug.LogWarningFormat("[AudioManager.Start] No hay AudioEffectsPool");
    ...
    private void Update()
    {
      for (int i = 0; i < effectsActive.Count; i++)
      {
        ...
        if (source != null && source.isPlaying == false && (effectsPaused.ContainsKey(source) == true && effectsPaused[source] == false) && source.spatialBlend != 1.0f)
        {
